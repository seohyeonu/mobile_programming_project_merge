...
public class MyRefrigerator extends AppCompatActivity {
    ImageButton return_main_Button, button_goto_addLayout;
...
//        });
        ...
        // 메인 액티비티에서 전환시 전달된 StringArrayList를 받아서 개인의 냉장고에 반영하고, 다시 메인화면으로 돌아갈 때 현재까지 수정된 냉장고의 재료를 다시 넘겨주자. (성제랑 같이 논의해서 짜면 되는 구간)
        Intent get_intent = getIntent();

        // 전달된 StringArrayList를 개인 냉장고 리스트에 반영해주기!
        if(get_intent.getStringArrayListExtra("MyList") != null){
            indivualList.addAll(get_intent.getStringArrayListExtra("MyList"));
            listAdapter.notifyDataSetChanged();
        }

        // 메인화면으로 돌아갈 때 현재 냉장고의 재료를 넘겨주자!
        return_main_Button = findViewById(R.id.returnMain);
        return_main_Button.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                get_intent.putStringArrayListExtra("UpdateList", indivualList);
    ...
    void showDialog(int targetListIndex, String targetListExpriation){
        ...
        commitButton.setOnClickListener(view -> {
            ...
            if(month >= 1 && month <=9) {
                if(day >= 1 && day <=9){
                    dateModified = year + ".0" + month + ".0" + day;
                }
                else {
                    dateModified = year + ".0" + month + "." + day;
                }
            }
            else {
                if(day >= 1 && day <=9){
                    dateModified = year + "." + month + ".0" + day;
                }
                else {
...
class MyRefrigeratorAdapter extends BaseAdapter{
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        "R.drawable.imageID|2024.11.20" 문자열을 "|" 기준으로 나누어서 아래와 같이 xml에 이미지와 텍스트를 수정해줍니다.
"         */
